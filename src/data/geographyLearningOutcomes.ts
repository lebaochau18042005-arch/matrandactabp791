...
const formulaRules: Array<{ pattern: RegExp; ids: string[]; guidance: string }> = [
  {
    pattern: /bản đồ|gps|tọa độ|vị trí|phạm vi|lãnh thổ|kinh tuyến|vĩ tuyến/i,
    ids: [
      'doi_hai_li_sang_km',
      'chieu_rong_vung_bien_km',
      'khoang_cach_thuc_te_tu_ti_le_ban_do',
...
const inferFormulaProfile = (lessonTitle: string, topic: string) => {
  const text = `${lessonTitle} ${topic}`;
  const rule = formulaRules.find(item => item.pattern.test(text));

  return rule ?? {
    ids: ['ti_trong', 'gia_tri_thanh_phan', 'toc_do_tang_truong', 'muc_tang', 'ti_le_phan_tram'],
    guidance: 'Dùng số liệu được cho trong đề để tính tỉ trọng, giá trị thành phần, tỉ lệ phần trăm, tốc độ tăng trưởng hoặc mức tăng phù hợp với nội dung bài.',
  };
};
...
export const getLessonAssessmentProfile = (
  ...
  const inferred = inferFormulaProfile(resolvedTitle, resolvedTopic);
