...
import { hasRenderableStimulusChart, hasRenderableStimulusTable } from './stimulusTable';
import {
  classifyAssessmentSource,
  hasDataYear,
  isApprovedSourceUrl,
  isForbiddenGeneratedSource,
  isPreciseTextbookReference,
} from '../data/assessmentSources';
import { formatNumericAnswer, getRoundingDigits } from './shortAnswer';
...
const metaStemPatterns = [
  /khi\s+(nhận\s+xét|phân\s+tích|đánh\s+giá).+thao\s+tác\s+nào/i,
  /để\s+(nhận\s+xét|phân\s+tích|đánh\s+giá).+thao\s+tác\s+nào/i,
  /thao\s+tác\s+nào.+(cần\s+thực\s+hiện|giúp\s+đánh\s+giá|phù\s+hợp)/i,
  /vận\s+dụng\s+kiến\s+thức\s+.+\s+nhằm\s+mục\s+đích/i,
  /kết\s+luận\s+nào.+khi\s+phân\s+tích/i,
  /nhận\s+xét\s+nào.+việc\s+vận\s+dụng/i,
  /trong\s+tình\s+huống\s+cần\s+đánh\s+giá/i,
  /thông\s+tin\s+nào.+xác\s+định\s+trước\s+tiên/i,
  /cách\s+(làm|nhận\s+xét|phân\s+tích|đánh\s+giá)\s+nào/i,
];
...
const stemTaskPatterns = [
  ...
  /nhận\s+xét\s+nào/i,
  /nhận\s+định\s+nào/i,
  /đặc\s+điểm(?:\s+\S+){0,2}\s+nào/i,
  /biểu\s+hiện\s+nào/i,
  /(?:vùng|khu\s+vực|lãnh\s+thổ|đối\s+tượng)\s+nào/i,
  /vai\s+trò(?:\s+\S+){0,2}\s+nào/i,
  /(?:thuận\s+lợi|khó\s+khăn)(?:\s+\S+){0,2}\s+nào/i,
  /xu\s+hướng(?:\s+\S+){0,2}\s+nào/i,
  /nguyên\s+nhân/i,
  /nhân\s+tố\s+nào/i,
  /yếu\s+tố\s+nào/i,
  /ý\s+nghĩa/i,
  ...
  /thông\s+tin\s+nào/i,
  /chủ\s+yếu\s+do/i,
  /tập\s+trung\s+(?:chủ\s+yếu\s+)?ở/i,
  /phân\s+bố\s+(?:chủ\s+yếu\s+)?ở/i,
  /thể\s+hiện\s+ở/i,
  /gây\s+khó\s+khăn/i,
  /đúng\s+hay\s+sai/i,
];

const acceptedCompletionStemEnding = /(chủ\s+yếu\s+do|nguyên\s+nhân\s+là|là\s+do|nhằm|thể\s+hiện\s+ở|biểu\s+hiện\s+ở|có\s+ý\s+nghĩa|tập\s+trung\s+(?:chủ\s+yếu\s+)?ở|phân\s+bố\s+(?:chủ\s+yếu\s+)?ở|cho\s+việc|là)\s*$/i;
...
const collectQuestionText = (q: QuizQuestion): string[] => {
  const values = [
    ...
    q.stimulus?.unit || '',
    q.stimulus?.source || '',
    q.stimulus?.sourceUrl || '',
    q.stimulus?.sourceDataset || '',
    q.stimulus?.dataYear || '',
    q.learningOutcome || '',
    q.competency || '',
...
const hasQuestionSource = (q: QuizQuestion) => hasText((q as any).sourceReference) || hasText(q.stimulus?.source);

const hasPreciseQuestionSource = (q: QuizQuestion) => {
  const sourceReference = (q as any).sourceReference as string | undefined;
  const stimulus = q.stimulus;
  const stimulusKind = classifyAssessmentSource(stimulus?.source);

  if (hasText(stimulus?.source)) {
    if (isPreciseTextbookReference(stimulus?.source)) return true;
    if (['nso_yearbook_2025', 'world_bank', 'faostat'].includes(stimulusKind)) {
      return isApprovedSourceUrl(stimulus?.sourceUrl, stimulusKind);
    }
    return false;
  }

  return isPreciseTextbookReference(sourceReference);
};

const validateStimulusSourceMetadata = (
  q: QuizQuestion,
  questionNumber: number,
  scope: 'table' | 'chart' | 'short_answer',
  issues: ValidationIssue[]
) => {
  const stimulus = q.stimulus;
  if (!stimulus || !hasText(stimulus.source)) return;

  if (isForbiddenGeneratedSource(stimulus.source)) {
    addIssue(issues, 'blocking', scope, 'Có nguồn tài liệu', `Nguồn "${stimulus.source}" là nguồn mô phỏng hoặc do hệ thống tự gán, không thể kiểm chứng.`, 'Thay bằng SGK ghi rõ lớp/bài hoặc nguồn thống kê chính thức: Niên giám thống kê Việt Nam 2025, World Bank WDI, FAOSTAT.', q, questionNumber);
    return;
  }

  const kind = classifyAssessmentSource(stimulus.source);
  if (kind === 'textbook') {
    if (!isPreciseTextbookReference(stimulus.source) && !isPreciseTextbookReference((q as any).sourceReference)) {
      addIssue(issues, 'blocking', scope, 'Có nguồn tài liệu', 'Nguồn SGK chưa ghi đủ lớp và bài/mục/trang tham chiếu.', 'Ghi theo dạng: SGK Địa lí 12 – Kết nối tri thức với cuộc sống, Bài ..., mục/trang ...', q, questionNumber);
    }
    return;
  }

  if (!['nso_yearbook_2025', 'world_bank', 'faostat'].includes(kind)) {
    addIssue(issues, 'blocking', scope, 'Có nguồn tài liệu', `Nguồn "${stimulus.source}" chưa thuộc nhóm nguồn được kiểm chứng cho số liệu.`, 'Dùng Niên giám thống kê Việt Nam năm 2025, World Bank WDI hoặc FAOSTAT và ghi đủ siêu dữ liệu truy xuất.', q, questionNumber);
    return;
  }

  if (!hasText(stimulus.sourceUrl)) {
    addIssue(issues, 'blocking', scope, 'Có nguồn tài liệu', 'Nguồn số liệu thiếu đường dẫn truy xuất chính thức.', 'Bổ sung URL trực tiếp trên miền gso.gov.vn/nso.gov.vn, worldbank.org hoặc fao.org/faostat.org.', q, questionNumber);
  } else if (!isApprovedSourceUrl(stimulus.sourceUrl, kind)) {
    addIssue(issues, 'blocking', scope, 'Có nguồn tài liệu', 'Đường dẫn nguồn không thuộc tên miền chính thức tương ứng.', 'Thay liên kết từ trang đăng lại bằng liên kết chính thức của Cục Thống kê, World Bank hoặc FAO.', q, questionNumber);
  }
  if (!hasText(stimulus.sourceDataset)) {
    addIssue(issues, 'blocking', scope, 'Đúng số liệu', 'Nguồn chưa ghi tên/số bảng, mã chỉ tiêu hoặc miền dữ liệu dùng để đối chiếu.', 'Bổ sung tên/số bảng của Niên giám, mã chỉ tiêu World Bank hoặc miền dữ liệu và phần tử của FAOSTAT.', q, questionNumber);
  }
  if (!hasDataYear(stimulus.dataYear)) {
    addIssue(issues, 'blocking', scope, 'Đúng số liệu', 'Ngữ liệu chưa khai báo năm dữ liệu có thể kiểm chứng.', 'Bổ sung dataYear, ví dụ: 2020; 2024 hoặc 2015–2024.', q, questionNumber);
  }
  if ((kind === 'world_bank' || kind === 'faostat') && !/^\d{4}-\d{2}-\d{2}$/.test(stimulus.accessedAt || '')) {
    addIssue(issues, 'warning', scope, 'Có nguồn tài liệu', 'Nguồn dữ liệu quốc tế chưa ghi ngày truy cập.', 'Bổ sung ngày truy cập theo định dạng YYYY-MM-DD để có thể tái kiểm tra phiên bản dữ liệu.', q, questionNumber);
  }
};
...
const referencesChart = (q: QuizQuestion) => q.stimulus?.type === 'chart' || collectStemAndStimulusText(q).some(text => /biểu\s+đồ|dựa vào biểu đồ|cho biểu đồ/i.test(text));
...
const validateStem = (q: QuizQuestion, questionNumber: number, issues: ValidationIssue[]) => {
  ...
  if (metaStemPatterns.some(pattern => pattern.test(questionText))) {
    addIssue(issues, 'blocking', 'question', 'Phần dẫn rõ ràng', 'Câu dẫn đang kiểm tra thao tác hoặc mẹo làm bài thay vì kiến thức, dữ liệu hay vấn đề Địa lí.', 'Nêu trực tiếp đối tượng, chỉ tiêu hoặc quan hệ Địa lí cần xác định; không hỏi học sinh về cách phân tích hay cách làm bài.', q, questionNumber);
  }
  ...
  if (q.type === 'multiple_choice' && !/[?？]\s*$/.test(questionText) && !acceptedCompletionStemEnding.test(questionText)) {
    addIssue(issues, 'warning', 'question', 'Phần dẫn rõ ràng', 'Câu trắc nghiệm chưa kết thúc bằng dấu hỏi và cũng chưa tạo thành câu dẫn hoàn tất bằng phương án.', 'Viết thành câu hỏi hoàn chỉnh hoặc câu dẫn trực tiếp kết thúc bằng “chủ yếu do”, “là”, “tập trung chủ yếu ở”... như quy cách đề tham khảo.', q, questionNumber);
  }
  if (/\bkhông\b.+\bkhông\b/i.test(questionText)) {
  ...
  if (/(?:nào\s+sau\s+đây|phát\s+biểu|nhận\s+định|đặc\s+điểm).{0,40}\b(?:không|chưa)\b|\b(?:không|chưa)\s+(?:đúng|phải|thuộc|thể\s+hiện)/i.test(questionText) && !/KHÔNG|CHƯA/.test(questionText)) {
...
export const validateDataTable = (q: QuizQuestion, questionNumber: number): ValidationIssue[] => {
  ...
  else if (!hasDataYear(stimulus.title)) addIssue(issues, 'blocking', 'table', 'Bảng số liệu đúng cấu trúc', 'Tên bảng chưa thể hiện năm hoặc giai đoạn của số liệu.', 'Viết tên bảng theo cấu trúc: chỉ tiêu + đối tượng/lãnh thổ + năm/giai đoạn.', q, questionNumber);
  ...
  if (!hasText(stimulus.source)) addIssue(issues, 'blocking', 'table', 'Có nguồn tài liệu', 'Bảng số liệu thiếu nguồn.', 'Ghi nguồn kiểm chứng được từ SGK, Niên giám thống kê Việt Nam 2025, World Bank WDI hoặc FAOSTAT.', q, questionNumber);
  validateStimulusSourceMetadata(q, questionNumber, 'table', issues);
...
export const validateChart = (q: QuizQuestion, questionNumber: number): ValidationIssue[] => {
  ...
  if (stimulus?.type !== 'chart') {
    addIssue(issues, 'blocking', 'chart', 'Biểu đồ đúng loại', 'Câu dẫn yêu cầu khai thác biểu đồ nhưng ngữ liệu chưa được khai báo là biểu đồ.', 'Chuyển loại ngữ liệu sang Biểu đồ và chọn đúng dạng biểu đồ.', q, questionNumber);
    return issues;
  }
  if (!stimulus || stimulus.type === 'none' || !hasRenderableStimulusChart(stimulus)) {
    addIssue(issues, 'blocking', 'chart', 'Biểu đồ đúng loại', 'Câu hỏi yêu cầu dựa vào biểu đồ nhưng dữ liệu chưa đủ để vẽ biểu đồ.', 'Bổ sung chartConfig có labels/datasets hoặc bảng dữ liệu gồm một cột nhãn và ít nhất một cột số.', q, questionNumber);
    return issues;
  }
  ...
  else if (!hasDataYear(stimulus.title)) addIssue(issues, 'blocking', 'chart', 'Biểu đồ đúng loại', 'Tên biểu đồ chưa thể hiện năm hoặc giai đoạn của số liệu.', 'Viết tên biểu đồ theo cấu trúc: chỉ tiêu + đối tượng/lãnh thổ + năm/giai đoạn.', q, questionNumber);
  ...
  if (!hasText(stimulus.source)) addIssue(issues, 'blocking', 'chart', 'Có nguồn tài liệu', 'Biểu đồ thiếu nguồn số liệu.', 'Ghi nguồn kiểm chứng được từ SGK, Niên giám thống kê Việt Nam 2025, World Bank WDI hoặc FAOSTAT.', q, questionNumber);
  validateStimulusSourceMetadata(q, questionNumber, 'chart', issues);
  return issues;
};
...
export const validateShortAnswer = (q: QuizQuestion, questionNumber: number, allowedFormulaNames: string[]): ValidationIssue[] => {
  ...
  if (!hasText(data.rounding) || getRoundingDigits(data.rounding) === null) addIssue(issues, 'blocking', 'short_answer', 'Đúng quy tắc làm tròn', 'Câu trả lời ngắn thiếu quy tắc làm tròn rõ ràng.', 'Ghi rõ làm tròn đến hàng đơn vị, 1 chữ số thập phân hoặc 2 chữ số thập phân.', q, questionNumber);
  ...
  if (/4\s*ô/i.test((data as any).answerFormat || '')) {
    const roundingDigits = getRoundingDigits(data.rounding);
    const maxCharacters = Number((data as any).maxCharacters || 4);
    if (maxCharacters !== 4) {
      addIssue(issues, 'blocking', 'short_answer', 'Đúng định dạng trả lời', `Phiếu 4 ô nhưng maxCharacters đang là ${maxCharacters}.`, 'Đặt maxCharacters = 4; nếu đáp án dài hơn, đổi đơn vị, dữ kiện hoặc quy tắc làm tròn.', q, questionNumber);
    }
    if (typeof data.correctAnswer === 'number' && roundingDigits !== null) {
      const answerForSheet = formatNumericAnswer(data.correctAnswer, data.rounding);
      if (answerForSheet.length > 4) {
        addIssue(issues, 'blocking', 'short_answer', 'Đúng định dạng trả lời', `Đáp án "${answerForSheet}" cần ${answerForSheet.length} ô, vượt giới hạn 4 ô.`, 'Đổi đơn vị, dữ kiện hoặc quy tắc làm tròn để đáp án tối đa 4 kí tự; không được tự cắt đáp án.', q, questionNumber);
      }
    }
  }
  if (!referencesTable(q) && !referencesChart(q)) {
    if (!hasText(q.stimulus?.source) && !isPreciseTextbookReference((q as any).sourceReference)) {
      addIssue(issues, 'blocking', 'short_answer', 'Có nguồn tài liệu', 'Dữ kiện tính toán chưa có nguồn cụ thể để kiểm chứng.', 'Gắn nguồn SGK ghi rõ lớp/bài/mục hoặc nguồn thống kê chính thức kèm URL, tên chỉ tiêu và năm dữ liệu.', q, questionNumber);
    } else {
      validateStimulusSourceMetadata(q, questionNumber, 'short_answer', issues);
    }
  }
  return issues;
};
...
const validateMultipleChoice = (q: QuizQuestion, questionNumber: number): ValidationIssue[] => {
  ...
  if (q.options.some((option, index) => option.key !== expectedKeys[index])) addIssue(issues, 'blocking', 'question', 'Có một đáp án đúng', 'Khóa phương án chưa theo đúng thứ tự A, B, C, D.', 'Chuẩn hóa bốn phương án lần lượt thành A, B, C, D trước khi chọn đáp án đúng.', q, questionNumber);
  ...
  if (optionTexts.some(text => !text)) addIssue(issues, 'blocking', 'question', 'Phương án nhiễu hợp lí', 'Có phương án đang để trống.', 'Viết đủ nội dung cho cả bốn phương án.', q, questionNumber);
  ...
  if (!hasText(q.explanation)) addIssue(issues, 'warning', 'question', 'Đúng thuật ngữ Địa lí', 'Câu trắc nghiệm chưa có giải thích đáp án.', 'Bổ sung giải thích ngắn, nêu căn cứ chọn đáp án đúng.', q, questionNumber);
  return issues;
};
...
export const validateQuestion = (q: QuizQuestion, questionNumber: number, allowedFormulaNames: string[]): ValidationIssue[] => {
  ...
  if (q.level === 'Nhận biết' && /nguyên\s+nhân|vì\s+sao|tại\s+sao|do\s+tác\s+động|gây\s+(?:khó\s+khăn|ảnh\s+hưởng|hệ\s+quả)|ý\s+nghĩa|vai\s+trò/i.test(q.question || '')) {
    addIssue(issues, 'warning', 'question', 'Đúng mức độ nhận thức', 'Câu gắn mức Nhận biết nhưng câu dẫn đang yêu cầu phân tích nguyên nhân, vai trò hoặc hệ quả.', 'Đổi câu dẫn thành yêu cầu nhận diện/trình bày đặc điểm, hoặc chuyển mức độ sang Thông hiểu nếu ma trận cho phép.', q, questionNumber);
  }
  if (!hasQuestionSource(q)) addIssue(issues, 'blocking', 'question', 'Có nguồn tài liệu', 'Câu hỏi chưa xác định được nguồn tài liệu.', 'Bổ sung nguồn SGK ghi rõ lớp/bài/mục hoặc nguồn số liệu chính thức có thể truy xuất.', q, questionNumber);
  else if (isForbiddenGeneratedSource((q as any).sourceReference) || isForbiddenGeneratedSource(q.stimulus?.source) || !hasPreciseQuestionSource(q)) addIssue(issues, 'blocking', 'question', 'Có nguồn tài liệu', 'Nguồn câu hỏi còn chung chung, là nguồn tự tạo hoặc chưa đủ thông tin để kiểm chứng.', 'Dùng SGK Địa lí ghi rõ lớp và bài/mục/trang; với số liệu, dùng nguồn chính thức kèm URL.', q, questionNumber);
  ...
  const hasStimulusContent = Boolean(q.stimulus && q.stimulus.type !== 'none' && (hasText(q.stimulus.content) || hasRenderableStimulusTable(q.stimulus)));
  if ((q.level === 'Vận dụng' || q.level === 'Vận dụng cao') && !hasStimulusContent && !referencesTable(q) && !referencesChart(q) && q.type !== 'short_answer' && !/tình huống|nhận xét|phân tích|so sánh|giải thích|vận dụng|đánh giá/i.test(q.question || '')) addIssue(issues, 'warning', 'question', 'Đúng mức độ nhận thức', 'Câu đang gắn mức vận dụng nhưng chưa có dữ liệu, tình huống hoặc thao tác xử lí rõ.', 'Bổ sung bảng/biểu đồ/tình huống hoặc đổi mức độ nhận thức phù hợp.', q, questionNumber);
...
export const validateExam = (quiz: GeneratedQuiz): ValidationIssue[] => {
  ...
  const normalizedStems = questions.filter(q => q.type !== 'true_false').map(q => normalize(q.question));
  const duplicateStems = normalizedStems.filter((stem, index) => stem && normalizedStems.indexOf(stem) !== index);
  if (duplicateStems.length > 0) addIssue(issues, 'blocking', 'exam', 'Phần dẫn rõ ràng', 'Đề có câu dẫn bị lặp nguyên văn.', 'Viết lại để mỗi câu đo một yêu cầu riêng; không lặp câu dẫn giữa các câu.');
  const answerCounts = questions
    .filter(q => q.type === 'multiple_choice')
    .reduce<Record<string, number>>((counts, question) => {
      counts[question.correctAnswer] = (counts[question.correctAnswer] || 0) + 1;
      return counts;
    }, { A: 0, B: 0, C: 0, D: 0 });
  const answerValues = ['A', 'B', 'C', 'D'].map(key => answerCounts[key] || 0);
  const answerSpread = Math.max(...answerValues) - Math.min(...answerValues);
  if (answerValues.some(count => count === 0) || answerSpread > 4) {
    addIssue(issues, 'blocking', 'exam', 'Đúng cấu trúc đề', `Phân bố đáp án đúng A/B/C/D đang là ${answerValues.join('/')}, dễ tạo quy luật hoặc lệch đáp án.`, 'Đảo vị trí phương án để phân bố đáp án đúng tương đối cân bằng giữa A, B, C, D.');
  } else if (answerSpread > 2) {
    addIssue(issues, 'warning', 'exam', 'Đúng cấu trúc đề', `Phân bố đáp án đúng A/B/C/D đang là ${answerValues.join('/')}, chưa cân bằng.`, 'Điều chỉnh vị trí phương án để chênh lệch giữa các chữ cái không quá 2 câu.');
  }
  return issues;
};
